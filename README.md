...
The package also allows you to: 1) Add the question text as attribute to
the variable

2)  Create a new variable to the dataset that is a list-column with
    `tidygraph`-objects. For this, the `tidygraph` package needs to be
    installed.

3)  Add background variables of the respondents. For this to work, you
...
4)  Remove the variables on the time it took respondents to answer each
    question. This information is probably less relevant to many
    researchers.

The below code is therefore probably what most researchers are after.

``` r
data <- produce_data(add_question_text = TRUE,
                     tidygraph_col = TRUE,
...
- Buijs, V. L., & Stulp, G. (2022). Friends, family, and family friends:
...
- Stulp, G & Barrett, L. Do data from large personal networks support
  cultural evolutionary ideas about kin and fertility? Social Sciences
  10(5):177. <https://doi.org/10.3390/socsci10050177>.
